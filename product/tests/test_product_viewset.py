...
class TestProductListExcludesBooks:
    ...
    def test_book_still_appears_in_books_endpoint(self, api_client):
        Book.objects.create(
            title="Duna", price="45.00", stock=5,
            author="Frank Herbert", isbn="9788576570059", published_at="1965-08-01",
        )
...
class TestCategoryFilterExcludesInactive:
    def test_inactive_category_not_assignable_via_product_creation(self, authenticated_client):
        inactive = Category.objects.create(name="Descontinuada", slug="descontinuada", is_active=False)
...
class TestPaginationLimits:
    ...
    def test_page_size_respects_max_page_size(self, api_client):
        ...
        assert len(response.data["results"]) <= 100  # max_page_size do ProductPagination